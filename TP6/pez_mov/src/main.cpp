...
void updateControlPointsAround(Spline &spline, int ctrl_pt) {
	...
	glm::vec3 l_i_next = spline.getControlPoint(ctrl_pt+3);
	glm::vec3 l_i_prev = spline.getControlPoint(ctrl_pt-3);
	glm::vec3 l_i = (l_i_next - l_i_prev)*0.5f;
	
	glm::vec3 P_i = spline.getControlPoint(ctrl_pt);
	glm::vec3 P_i_prev = P_i - (l_i/3.f);
	glm::vec3 P_i_next = P_i + (l_i/3.f);
	
	spline.setControlPoint(ctrl_pt+1,P_i_next);
	spline.setControlPoint(ctrl_pt-1,P_i_prev);
}
...
glm::mat4 getTransform(const Spline &spline, double t) {
	...
	float ang = atan2(deriv.x,deriv.z);
	...
	*/
	...
	m = glm::rotate(m,ang-3.14f*0.5f,{0.f,1.f,0.f});
	m = glm::scale(m,{0.3f,0.3f,0.3f});
