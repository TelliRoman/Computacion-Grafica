...
int main() {
	...
	do {
		...
		glStencilFunc(GL_NEVER,0,~0); //Como se pasa el stencil test
		drawFloor(true);
		...
		glStencilFunc(GL_EQUAL,1,~0); //Si es piso dibujar
		drawObject(reflection);
		drawFloor(true);
		
		glStencilFunc(GL_EQUAL,2,~0); //Si es version aplastada dibujar
		drawFloor(false);
		
		glDisable(GL_STENCIL_TEST); //Desactivar stencil test
		drawObject(identity);
		drawLight();
